...
import matplotlib.patheffects as path_effects  # <--- 新增导入，用于文字描边
import configparser
import sys
import numpy as np
...
def load_config() -> Tuple[str, str, str, str, int, int, List[Tuple[str, float]]]:
    ...
    if not os.path.exists(config_file):
        ...
        default_config['Portfolio'] = {'AAPL': '100', 'GOOGL': '50'}
        default_config['Settings'] = {'max_retries': '3', 'retry_delay_seconds': '5'}
...
def plot_history_graph(output_filename: str):
    ...
    # --- 核心修改 1: 绘制堆叠图 ---
    # 使用 'viridis' colormap 获取一组颜色，确保每次运行颜色一致
    colors = plt.cm.viridis(np.linspace(0, 1, len(stock_columns)))
    ax.stackplot(df.index, [df[col] for col in stock_columns], alpha=0.8, colors=colors)
    ...
    # 将标签定位在图表 x-轴 25% 的位置，避免被边缘切掉
    label_idx = int(len(df.index) * 0.25)
    total_value_at_label_point = df['total_value'].iloc[label_idx]
    x_label_pos = df.index[label_idx]

    for i, col in enumerate(stock_columns):
        y_values = df[col].values

        # 计算标签的垂直中心位置
        y_label_pos = y_bottom[label_idx] + y_values[label_idx] / 2

        # 仅为足够厚的区域添加标签，避免拥挤
        if y_values[label_idx] > total_value_at_label_point * 0.03:
            # 创建文字描边效果，确保在任何背景色上都清晰可见
            text_outline = [path_effects.withStroke(linewidth=3, foreground='black')]

            ax.text(x_label_pos, y_label_pos, col,
                    ha='center', va='center', color='white',
                    fontsize=12, fontweight='bold',
                    path_effects=text_outline)  # 应用描边效果
